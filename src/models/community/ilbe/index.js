...
const generateURL = async (currentPage = 1, keyword) => {
  const host = 'https://www.ilbe.com'
  const url = `${host}/search?docType=doc&searchType=title_content&page=${currentPage}&q=${encodeURI(
    keyword,
  )}`
  console.log(url)
  return url
...
const getPostsInfoInListPage = async $ => {
  const infoInListPage = $('div.search-list > ul > li')
    .toArray()
    .map((row, index) => {
      return {
        date: $(row)
...
const goToPostPageAndGetInfo = async (page, data, link) => {
  ...
  const item = {
    ...
    date: $('div.count > span.date')
      .text()
      .substring(0, 10),
    title: filter($('div.post-header > h3 > a').text()),
    username: filter($('div.post-header > span > a').text()),
    content: filter($('div.post-content').text()),
    click: '',
    link,
    site: data.site,
...
const getPageCount = async page => {
  const totalPostCount = await page.evaluate(() =>
    document
      .querySelectorAll('.search-result > strong')[0]
      .innerText.replace(/\,/g, ''),
  )
  const pageSize = 10
